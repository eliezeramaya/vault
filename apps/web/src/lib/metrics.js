...
export const endFocus = (session, { actual_min, interruptions = 0 }) => {
  ...
  const diffMin =
    typeof actual_min === 'number'
      ? actual_min
      : Math.max(1, Math.round((ended_at - started_at) / 60000))
...
export const streakDays = () => {
  ...
  const daysSet = new Set(
    sessions
      ...
      .map(d => formatYmd(new Date(d.getFullYear(), d.getMonth(), d.getDate()))),
  )
...
export const pctQ12Completed = (wk = weekKey()) => {
  ...
  const scheduledThisWeek = tasks.filter(
    t =>
      t.scheduled_for &&
      inWeek(t.scheduled_for) &&
      (t.quadrant === Quadrant.Q1 || t.quadrant === Quadrant.Q2),
  )
...
export const ttfQ2Median = (wk = weekKey()) => {
  ...
  for (const t of tasks) {
    ...
    const mins = Math.max(
      0,
      Math.round((done - (t.scheduled_for ? new Date(t.scheduled_for) : created)) / 60000),
    )
    diffs.push(mins)
  }
...
export const deriveNextBestTask = () => {
  ...
  const byPref = (a, b) =>
    (b.importance || 0) - (a.importance || 0) || new Date(a.created_at) - new Date(b.created_at)
  ...
  try {
    ...
    if (q1n.length)
      return {
        id: null,
        title: q1n[0].text || 'Nota Q1',
        quadrant: Quadrant.Q1,
        importance: q1n[0].priority || 3,
      }
    const q2n = notes.filter(n => getQ(n) === Quadrant.Q2)
    if (q2n.length)
      return {
        id: null,
        title: q2n[0].text || 'Nota Q2',
        quadrant: Quadrant.Q2,
        importance: q2n[0].priority || 3,
      }
  } catch (e) {
    /* ignore suggestions parse */
  }
  return null
}
...
export const scheduleQ2BlocksSuggestion = (date = new Date()) => {
  ...
  const t2d = new Date(base)
  t2d.setMinutes(t2d.getMinutes() + 30)
  const t2 = addTask({
    title: 'Bloque Q2',
    quadrant: Quadrant.Q2,
    estimate_min: 25,
    scheduled_for: t2d.toISOString(),
  })
  logEvent('suggestion_accepted', {
    type: 'schedule_q2',
    slots: [t1.scheduled_for, t2.scheduled_for],
  })
  return [t1, t2]
}
...
export const getMotivationalInsights = () => {
  ...
  const _weeklyPomodoros = history.filter(p => new Date(p.date) > weekAgo).length
  const _monthlyPomodoros = history.filter(p => new Date(p.date) > monthAgo).length
...
const calculatePersonalBests = history => {
  ...
  const bestDay = {
    ...
    date: Object.keys(dailyCounts).reduce(
      (a, b) => (dailyCounts[a] > dailyCounts[b] ? a : b),
      Object.keys(dailyCounts)[0],
    ),
  }
  ...
  const bestWeek = {
    ...
    week: Object.keys(weeklyCount).reduce(
      (a, b) => (weeklyCount[a] > weeklyCount[b] ? a : b),
      Object.keys(weeklyCount)[0],
    ),
  }
...
const getBaseMetrics = () => {
  ...
  const _sessions = listSessions().filter(s => s.source === 'pomodoro')
