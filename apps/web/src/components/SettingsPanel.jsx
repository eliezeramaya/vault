...
function ThemeOptionButton({ option, selected, onSelect }){
  ...
  const mobileButton = useMobileButton({ onPress: onSelect, hapticFeedback: true, rippleEffect: true });
  return (
    <button
      type="button"
    ...
    >
      ...
      <div style={{ fontSize:12, opacity:0.8, lineHeight:1.3, color: selected ? 'var(--on-primary)' : 'var(--text-secondary)' }}>
        {option.description}
      </div>
      {selected && (
        <div style={{ fontSize:11, fontWeight:600, background: 'rgba(255,255,255,0.2)', padding:'2px 8px', borderRadius:6 }}>
          ✓ Activo
        </div>
      )}
    </button>
...
export default function SettingsPanel({ theme, onThemeToggle }){
  ...
  const { containerRef, focusElement } = useKeyboardNavigation({
    ...
    customShortcuts: {
      '1': () => { focusElement('[data-theme="light"]'); return true },
      '2': () => { focusElement('[data-theme="dark"]'); return true },
      'r': () => { resetToSystemTheme(); return true },
      'R': () => { resetToSystemTheme(); return true },
      't': () => { onThemeToggle(); return true },
      'T': () => { onThemeToggle(); return true }
    },
  ...
  return (
    <section ref={containerRef} aria-label="Ajustes" style={{
      ...
      borderRadius:16, boxShadow: 'var(--elevation-1)'
    }} tabIndex={-1}>
      ...
      <div style={{marginBottom:32}}>
        ...
        <div style={{
          ...
          <div>
            ...
            <div style={{fontSize:14, opacity:0.8, color:'var(--text-secondary)'}}>
              Tu sistema operativo está configurado en modo {systemTheme === 'dark' ? 'oscuro' : 'claro'}
            </div>
          </div>
        </div>
        ...
        <div style={{marginBottom:16}}>
          <label htmlFor="theme-grid" style={{
          ...
          <div id="theme-grid" className="settings-grid" style={{display:'grid', gridTemplateColumns:'1fr 1fr', gap:12}}>
          ...
          <div style={{
            ...
            <button
              ...
              style={{
                ...
                outline:'none', minHeight:'44px', // Touch-friendly
                transition: 'all 0.2s ease'
              }}
              onFocus={(e) => {
                e.target.style.outline = '2px solid var(--accent)'
            ...
            >
              <Monitor size={18} style={{marginRight:8, verticalAlign:'middle'}} aria-hidden="true" />
              Seguir tema del sistema
            </button>
            <div style={{
              fontSize:12, opacity:0.7, marginTop:8, color:'var(--text-secondary)'
            }}>
            ...
            <div style={{display:'flex', flexWrap:'wrap', gap:8}}>
              <button
                ...
                style={{
                  ...
                  padding:'10px 16px', fontSize:12, cursor:'pointer', outline:'none',
      ...
      <div style={{
        background:'var(--panel-bg)', border:'1px solid var(--panel-border)',
        borderRadius:12, padding:16, fontSize:14, color:'var(--text-secondary)'
      }}>
        <div style={{fontWeight:600, marginBottom:8, color:'var(--text)'}}>
          💡 Consejo:
        </div>
        <div>
          Puedes cambiar el tema rápidamente desde cualquier lugar usando el botón en la barra superior, 
          o presiona <kbd style={{
            background:'var(--surface)', border:'1px solid var(--surface-border)',
            padding:'2px 6px', borderRadius:4, fontSize:12, fontFamily:'monospace'
          }}>H</kbd> para ver todos los atajos disponibles.
        </div>
      </div>
