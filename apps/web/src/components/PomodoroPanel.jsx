...
export default function PomodoroPanel() {
  ...
  const [workMin, setWorkMin] = useState(() =>
    loadNumber('pomo_work_min', testMode ? 0.1 : 25, testMode)
  )
  const [shortBreakMin, setShortBreakMin] = useState(() =>
    loadNumber('pomo_short_min', testMode ? 0.1 : 5, testMode)
  )
  const [longBreakMin, setLongBreakMin] = useState(() =>
    loadNumber('pomo_long_min', testMode ? 0.1 : 15, testMode)
  )
  ...
  const [notificationsEnabled, setNotificationsEnabled] = useState(() =>
    loadBool('pomo_notifications', true)
  )
  ...
  useEffect(() => {
    try {
      localStorage.setItem('pomo_work_min', String(workMin))
    } catch {}
  }, [workMin])
  useEffect(() => {
    try {
      localStorage.setItem('pomo_short_min', String(shortBreakMin))
    } catch {}
  }, [shortBreakMin])
  useEffect(() => {
    try {
      localStorage.setItem('pomo_long_min', String(longBreakMin))
    } catch {}
  }, [longBreakMin])
  useEffect(() => {
    try {
      localStorage.setItem('pomo_interval', String(interval))
    } catch {}
  }, [interval])
  useEffect(() => {
    try {
      localStorage.setItem('pomo_auto_break', JSON.stringify(autoStartBreak))
    } catch {}
  }, [autoStartBreak])
  useEffect(() => {
    try {
      localStorage.setItem('pomo_auto_next', JSON.stringify(autoStartNext))
    } catch {}
  }, [autoStartNext])
  useEffect(() => {
    try {
      localStorage.setItem('pomo_notifications', JSON.stringify(notificationsEnabled))
    } catch {}
  }, [notificationsEnabled])
  useEffect(() => {
    try {
      localStorage.setItem('pomo_audio', JSON.stringify(audioEnabled))
    } catch {}
  }, [audioEnabled])
  useEffect(() => {
    try {
  ...
  useEffect(() => {
    try {
      localStorage.setItem('pomo_settings_open', JSON.stringify(settingsOpen))
    } catch {}
  }, [settingsOpen])
  ...
  // Derived values
  const dots = useMemo(
    () => Array.from({ length: interval }, (_, i) => i < completedInSet),
    [interval, completedInSet]
  )
  ...
  const onPhaseComplete = () => {
    if (phase === 'work') {
      ...
      showNotification(
        '🎉 ¡Pomodoro completado!',
        `¡Excelente trabajo! Hora de ${isLongBreak ? 'un descanso largo' : 'un descanso corto'}`
      )
  ...
  const showNotification = (title, body) => {
    ...
    // Use Vite base so icons resolve in dev ("/") and prod ("/vault/")
    const base =
      (typeof import.meta !== 'undefined' && import.meta.env && import.meta.env.BASE_URL) || '/'
    const iconPath = `${base}icons/icon-192x192.png`
  ...
  return (
    <div
      style={{
        // Safe-area aware margins and bottom space to avoid mobile nav overlap
        maxWidth: '800px',
        margin:
          'max(16px, env(safe-area-inset-top)) max(10px, env(safe-area-inset-right)) 96px max(10px, env(safe-area-inset-left))',
        padding: 16,
        display: 'flex',
            ...
            >
              ...
              <div
                style={{
                  ...
                  lineHeight: 1,
                  animation:
                    leftSec <= 60 && phase === 'work'
                      ? 'countdown-warning 1s ease-in-out infinite'
                      : 'none',
                }}
                  ...
                  <Tooltip content="Duración de cada sesión de trabajo enfocado. Recomendado: 25 minutos para máxima concentración.">
                    <label style={labelStyle}>
                      ...
                      <input
                        ...
                        value={workMin}
                        onChange={(e) =>
                          setWorkMin(clampNum(e.target.value, testMode ? 0.1 : 10, 60, testMode))
                        }
                        style={inputStyle}
                      />
                    </label>
                  ...
                  <Tooltip content="Descanso entre sesiones de trabajo. Ideal para relajar la mente manteniendo el ritmo.">
                    <label style={labelStyle}>
                      ...
                      <input
                        ...
                        onChange={(e) =>
                          setShortBreakMin(
                            clampNum(e.target.value, testMode ? 0.1 : 3, 30, testMode)
                          )
                        }
                        style={inputStyle}
                      />
                  ...
                  <Tooltip content="Descanso más largo después de completar un set completo. Tiempo para recargar energías.">
                    <label style={labelStyle}>
                      ...
                      <input
                        ...
                        onChange={(e) =>
                          setLongBreakMin(
                            clampNum(e.target.value, testMode ? 0.1 : 5, 60, testMode)
                          )
                        }
                        style={inputStyle}
                      />
              ...
              >
                ...
                >
                  <label style={labelStyle}>
                    ...
                    >
                      ...
                      <option value={Quadrant.Q3}>
                        🟡 Q3 - Urgente, No importante (Interrupciones)
                      </option>
                      <option value={Quadrant.Q4}>
                        ⚪ Q4 - No urgente, No importante (Distracción)
                      </option>
                    </select>
                  </label>
                </Tooltip>
                ...
                >
                  ...
                  >
                    ...
                    <div style={{ fontSize: '10px', color: '#14532d' }}>
