...
test.describe('Equirectangular plane mapping', () => {
  test('chips expose lat/lon and update correctly when dragged', async ({ page }) => {
    ...
    if (!(await chip.isVisible({ timeout: 2000 }).catch(() => false))) {
      test.skip(true, 'Projection chip not present; feature possibly disabled')
    }
    // Espera a que la capa de heatmap publique puntos
    await page
      .waitForFunction(
        () => Array.isArray((window as any).__heatPts) && (window as any).__heatPts.length > 0,
        null,
        { timeout: 5000 }
      )
      .catch(() => {})
    ...
    await page
      ...
      .catch(() => {})
    const { heatPts, nodeCoords } = await page.evaluate(() => ({
      heatPts: (window as any).__heatPts,
      nodeCoords: (window as any).__nodeCoords,
    }))
