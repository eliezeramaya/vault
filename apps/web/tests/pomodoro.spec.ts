...
test.describe('Pomodoro panel', () => {
  test('should be accessible and have functional UI', async ({ page }) => {
    ...
    // Verify Pomodoro panel is rendered with expected elements
    ...
    const startButton = page.getByText('Comenzar sesión de enfoque')
    ...
    const settingsSection = page.getByText('⚙️ Configuración')
    ...
    await expect(page.getByText('Pausar sesión')).toBeVisible()
    await expect(page.getByText('Finalizar sesión')).toBeVisible()
    ...
    await page.getByText('Pausar sesión').click()
    await expect(page.getByText('Reanudar sesión')).toBeVisible()
    await expect(page.locator('span').filter({ hasText: 'PAUSADO' })).toBeVisible()
    
    // Test resume
    await page.getByText('Reanudar sesión').click()
    await expect(page.getByText('Pausar sesión')).toBeVisible()
    await expect(page.locator('span').filter({ hasText: 'ENFOQUE' })).toBeVisible()
    
    // Test reset
    await page.getByText('Finalizar sesión').click()
    await expect(page.getByText('Comenzar sesión de enfoque')).toBeVisible()
