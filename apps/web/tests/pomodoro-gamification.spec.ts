...
test.describe('Pomodoro Stats Gamification', () => {
  test.beforeEach(async ({ page }) => {
    ...
    // Use relative navigation (Playwright baseURL is http://localhost:4173/vault/)
    await page.goto('./?pomo_test=1')
    await page.evaluate(() => {
      localStorage.clear()
      // Set test mode
    ...
    await page.reload()
    // Dismiss welcome / overlay dialog if present so it doesn't block clicks
  ...
  test('shows achievement categories and filtering', async ({ page }) => {
    ...
    await expect(
      page.locator('text=Primer Paso').locator('xpath=ancestor::div[contains(@style, "grid")]')
    ).toBeVisible()
  })
